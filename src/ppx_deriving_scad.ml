...
let accessor_impl (ld : label_declaration) =
  ...
  (* let func_name =
   *   { loc; txt = ld.pld_name.txt  }
   * in *)
  (* Ast.pexp_let Nonrecursive *)
  (* TODO: can I access a field in x using the name of the field with metaquot? *)
  [%stri let [%p Ast.pvar ld.pld_name.txt] = [%expr fun x -> x]]
...
let generate_impl ~ctxt (_rec_flag, type_declarations) =
  ...
  let f (td : type_declaration) =
    ...
    | { ptype_kind = Ptype_record fields; _ } -> List.map fields ~f:accessor_impl
  in
