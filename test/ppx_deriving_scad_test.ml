...
type vec_pair = { reg : Vec3.t; unit : Vec3.t [@scad.unit] } [@@deriving scad]

type with_ignored = { vector : Vec3.t; ignored : int [@scad.ignore] }
[@@deriving scad]
...
module PolyType : sig
  type 'a p = { a : 'a [@scad.ignore]; v : Vec3.t } [@@deriving scad]
...
let%test "unit_prevents_translate" =
  let a = { reg = (5., 5., 0.); unit = (0., 1., 0.) } and p = (0., 5., 0.) in
  let trans = translate_vec_pair p a in
  Vec3.equal trans.reg (Vec3.translate p a.reg) && Vec3.equal trans.unit a.unit

let%test "ignored" =
  let a = { vector = (1., 2., 3.); ignored = 0 } and p = (1., 1., 1.) in
  let trans = translate_with_ignored p a in
  Vec3.equal trans.vector (Vec3.translate p a.vector)
  && a.ignored = trans.ignored
...
let%test "translate_opt_opt" =
  let a = OptOpt.{ vec = Some (Some Vec3.zero) } and p = (1., 1., 1.) in
